...
class CycleGAN_seg:
    ...
    def model_setup(self):
        ...
        self.input_a = tf.placeholder(tf.float32, [1, model2.IMG_WIDTH,
                model2.IMG_HEIGHT, model2.IMG_CHANNELS], name='input_A')
        self.input_b = tf.placeholder(tf.float32, [1, model2.IMG_WIDTH,
                model2.IMG_HEIGHT, model2.IMG_CHANNELS], name='input_B')

        self.fake_pool_A = tf.placeholder(tf.float32, [None,
                model2.IMG_WIDTH, model2.IMG_HEIGHT,
                model2.IMG_CHANNELS], name='fake_pool_A')
        self.fake_pool_B = tf.placeholder(tf.float32, [None,
                model2.IMG_WIDTH, model2.IMG_HEIGHT,
                model2.IMG_CHANNELS], name='fake_pool_B')
        self.fake_pool_A_mask = tf.placeholder(tf.float32, [None,
                model2.IMG_WIDTH, model2.IMG_HEIGHT,
                model2.IMG_CHANNELS], name='fake_pool_A_mask')
        self.fake_pool_B_mask = tf.placeholder(tf.float32, [None,
                model2.IMG_WIDTH, model2.IMG_HEIGHT,
                model2.IMG_CHANNELS], name='fake_pool_B_mask')

        self.manual_seg = tf.placeholder(tf.float32, [None,
                model2.IMG_WIDTH, model2.IMG_HEIGHT, 1], name='seg_map')

        self.auto_seg = tf.placeholder(tf.float32, [None,
                model2.IMG_WIDTH, model2.IMG_HEIGHT, 1], name='seg_auto'
                )
        ...
        self.learning_rate = tf.placeholder(tf.float32, shape=[],
                name='lr')
        self.transition_rate = tf.placeholder(tf.float32, shape=[],
                name='tr')
        ...
        outputs = model2.get_outputs(inputs, skip=self._skip)
        ...
        self.prob_fake_pool_a_is_real = \
            outputs['prob_fake_pool_a_is_real']
        self.prob_fake_pool_b_is_real = \
            outputs['prob_fake_pool_b_is_real']
        ...
        with open(os.path.join(self._output_dir, 'epoch_' + str(epoch)
            ...
            for i in range(0, self._num_imgs_to_save):
                ...
                (
                    ...
                    self.masks,
                    self.prediction,
                    ], feed_dict={  # self.manual_seg: inputs['images_c'],
                ...
                for (name, tensor) in zip(names, tensors):
                    image_name = name + str(epoch) + '_' + str(i) \
                        + '.jpg'
                    if 'mask_' in name:
                        imsave(os.path.join(self._images_dir,
                               image_name), np.squeeze(tensor[0]))
                    else:

                        imsave(os.path.join(self._images_dir,
                               image_name), ((np.squeeze(tensor[0])
                               + 1) * 127.5).astype(np.uint8))
                    v_html.write('<img src="' + os.path.join('imgs',
                                 image_name) + '">')
                v_html.write('<br>')
    ...
    def save_images_bis(self, sess, epoch):
        ...
        with open(os.path.join(self._output_dir, 'results_'
                  + str(epoch) + '.html'), 'w') as v_html:
            ...
            for i in range(0, self._num_imgs_to_save):
                print 'Saving image {}/{}'.format(i,
                        self._num_imgs_to_save)
                inputs = sess.run(self.inputs)
                (fake_A_temp, fake_B_temp, masks, masked_ims,
                 pred_seg) = sess.run([self.fake_images_a,
                        self.fake_images_b, self.masks,
                        self.masked_ims, self.prediction], feed_dict={
                    self.input_a: inputs['images_i'],
                    self.input_b: inputs['images_j'],
                ...
                print (np.min(inputs['images_c']),
                       np.max(inputs['images_c']))
                ...
                ssim_list.append(compare_ssim(IM1, IM2,
                                 multichannel=True))
            ...
            print 'mean=%f,std=%f' % (100 - np.mean(total_np) * 100,
                    np.std(total_np))
            print total_mse
            print len(total_mse)
            ...
            print 'DSC_Lesion = %f ,STD = %f' \
                % (np.mean(total_dsc_lesion) * 100,
                   np.std(total_dsc_lesion))

            if infant == 1:
                total_np_WM = np.array(total_mse_WM)
                print 'mean_WM=%f,std_WM=%f' % (100
                        - np.mean(total_np_WM) * 100,
                        np.std(total_np_WM))

                total_np_GM = np.array(total_mse_GM)
                print 'mean_GM=%f,std_GM=%f' % (100
                        - np.mean(total_np_GM) * 100,
                        np.std(total_np_GM))

                total_np_CSF = np.array(total_mse_CSF)
                print 'mean_CSF=%f,std_CSF=%f' % (100
                        - np.mean(total_np_CSF) * 100,
                        np.std(total_np_CSF))
              ...
              # print('mean_dsc_BG  = %f   ,dsc_BG = %f'%(np.mean(total_dsc_BG)*100,np.std(total_dsc_BG)))
                ...
                print 'total_dsc_WM  = %f ,STD = %f' \
                    % (np.mean(total_dsc_WM) * 100,
                       np.std(total_dsc_WM))
                print 'total_dsc_GM  = %f ,STD = %f' \
                    % (np.mean(total_dsc_GM) * 100,
                       np.std(total_dsc_GM))
                print 'total_dsc_CSF = %f ,STD = %f' \
                    % (np.mean(total_dsc_CSF) * 100,
                       np.std(total_dsc_CSF))

                print 'total_asd_WM  = %f ,STD = %f' \
                    % (np.mean(total_asd_WM), np.std(total_asd_WM))
                print 'total_asd_GM  = %f ,STD = %f' \
                    % (np.mean(total_asd_GM), np.std(total_asd_GM))
                print 'total_asd_CSF = %f ,STD = %f' \
                    % (np.mean(total_asd_CSF), np.std(total_asd_CSF))

                print 'total_HD95_WM = %f ,STD = %f' \
                    % (np.mean(total_HD95_WM), np.std(total_HD95_WM))
                print 'total_HD95_GM = %f ,STD = %f' \
                    % (np.mean(total_HD95_GM), np.std(total_HD95_GM))
                print 'total_HD95_CSF= %f ,STD = %f' \
                    % (np.mean(total_HD95_CSF), np.std(total_HD95_CSF))
    ...
    def train(self):
        ...
        self.inputs = data_loader2.load_data(self._dataset_name,
                self._size_before_crop, False, self._do_flipping)
        ...
        max_images = \
            cyclegan_datasets.DATASET_TO_SIZES[self._dataset_name]
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(sess.run(self.global_step),
                               self._max_step):
                print ('In the epoch ', epoch)

                saver.save(sess, os.path.join(self._output_dir, 'AGGAN'
                           ), global_step=epoch)
                ...
                for i in range(0, max_images):  #
                    ...
                    if just_seg == 1:
                        (_, summary_seg_str) = \
                            sess.run([self.seg_trainer,
                                self.segmentation_loss_sum], feed_dict={
                            self.input_a: inputs['images_i'],
                            self.input_b: inputs['images_j'],
                            self.manual_seg: inputs['images_c'],
                        ...
                        writer.add_summary(summary_seg_str, epoch
                                * max_images + i)
                    else:

                      # Optimizing the G_A network

                        (_, fake_B_temp, smask_a, summary_str,
                         summary_str1) = sess.run([to_train_A,
                      ...
                      # writer.add_summary(summary_str1, epoch * max_images + i)
                        ...
                        fake_B_temp1 = \
                            self.fake_image_pool(self.num_fake_inputs,
                                fake_B_temp, smask_a,
                                self.fake_images_B)

                      # Optimizing the D_B network

                        (_, summary_str) = sess.run([self.d_B_trainer,
                                self.d_B_loss_summ], feed_dict={
                            self.input_a: inputs['images_i'],
                            self.input_b: inputs['images_j'],
                            self.learning_rate: curr_lr,
                      ...
                      # Optimizing the G_B network
                        ...
                        (_, fake_A_temp, smask_b, summary_str) = \
                            sess.run([to_train_B, self.fake_images_a,
                                self.masks[1], self.g_B_loss_summ],
                                feed_dict={
                            self.input_a: inputs['images_i'],
                            self.input_b: inputs['images_j'],
                            self.learning_rate: curr_lr,
                      ...
                      # Optimizing the D_A network
                        ...
                        (_, mask_tmp__, summary_str) = \
                            sess.run([self.d_A_trainer, self.masks_,
                                self.d_A_loss_summ], feed_dict={
                            self.input_a: inputs['images_i'],
                            self.input_b: inputs['images_j'],
                            self.learning_rate: curr_lr,
...
def main():
    ...
    lambda_a = (float(config['_LAMBDA_A']) if '_LAMBDA_A'
                in config else 10.0)
    lambda_b = (float(config['_LAMBDA_B']) if '_LAMBDA_B'
                in config else 10.0)
    pool_size = (int(config['pool_size']) if 'pool_size'
                 in config else 50)

    to_restore = to_train == 2
    base_lr = (float(config['base_lr']) if 'base_lr'
               in config else 0.0002)
    max_step = (int(config['max_step']) if 'max_step'
                in config else 200)
    network_version = str(config['network_version'])
    dataset_name = str(config['dataset_name'])
    do_flipping = bool(config['do_flipping'])

    cyclegan_model_seg = CycleGAN_seg(
        pool_size,
        lambda_a,
        lambda_b,
        log_dir,
